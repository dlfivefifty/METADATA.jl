# Issue #3582 - check that all versions of a package newer than the
# grandfathered list below are at least minpkgver and furthermore have a
# requires file listing a minimum Julia version that is at least minjuliaver
maxver_list_3582 = Dict([ # List of grandfathered packages
    ("ACME", v"0.6.2"),
    ("AMD", v"0.1.0"),
    ("AMQPClient", v"0.0.2"),
    ("ASCIIPlots", v"0.0.3"),
    ("ASTInterpreter", v"0.0.4"),
    ("AWS", v"0.4.0"),
    ("AWSClusterManagers", v"0.1.0"),
    ("AWSCore", v"0.2.3"),
    ("AWSEC2", v"0.2.0"),
    ("AWSIAM", v"0.2.0"),
    ("AWSLambda", v"0.2.0"),
    ("AWSS3", v"0.2.0"),
    ("AWSSDB", v"0.2.0"),
    ("AWSSDK", v"0.2.0"),
    ("AWSSES", v"0.2.1"),
    ("AWSSNS", v"0.2.1"),
    ("AWSSQS", v"0.2.1"),
    ("AbstractDomains", v"0.1.0"),
    ("AbstractFFTs", v"0.1.0"),
    ("AbstractTables", v"0.0.1"),
    ("AbstractTrees", v"0.1.0"),
    ("Accumulo", v"0.1.0"),
    ("ActiveAppearanceModels", v"0.1.2"),
    ("Actors", v"0.1.0"),
    ("ActuarialScience", v"0.0.3"),
    ("AdaptiveDistanceFields", v"0.0.1"),
    ("AdjacentFloats", v"0.0.6"),
    ("AffineInvariantMCMC", v"0.2.7"),
    ("AffineTransforms", v"0.2.3"),
    ("AlgebraicDiffEq", v"0.1.0"),
    ("AmplNLReader", v"0.1.0"),
    ("AmplNLWriter", v"0.3.0"),
    ("Anasol", v"0.2.8"),
    ("AndorSIF", v"0.0.3"),
    ("AnsiColor", v"0.0.2"),
    ("AppConf", v"0.1.1"),
    ("AppleAccelerate", v"0.2.1"),
    ("ApproxFun", v"0.6.1"),
    ("ArbFloats", v"0.1.17"),
    ("Arbiter", v"0.0.2"),
    ("ArcadeLearningEnvironment", v"0.0.1"),
    ("Arduino", v"0.1.2"),
    ("ArgCheck", v"0.2.1"),
    ("ArgParse", v"0.5.0"),
    ("ArrayFire", v"0.1.0"),
    ("ArrayViews", v"0.6.4"),
    ("Arrowhead", v"0.0.1"),
    ("AstroLib", v"0.1.0"),
    ("AtariAlgos", v"0.0.1"),
    ("Atom", v"0.6.5"),
    ("AudioIO", v"0.1.1"),
    ("Augmentor", v"0.1.0"),
    ("Augur", v"0.1.1"),
    ("AutoDiffSource", v"0.2.3"),
    ("AutoGrad", v"0.0.10"),
    ("AutoHashEquals", v"0.1.2"),
    ("AutoTypeParameters", v"0.0.3"),
    ("Automa", v"0.3.1"),
    ("Autoreload", v"0.2.0"),
    ("AverageShiftedHistograms", v"0.5.2"),
    ("Avro", v"0.0.3"),
    ("AxisAlgorithms", v"0.2.0"),
    ("AxisArrayUtils", v"0.0.1"),
    ("AxisArrays", v"0.1.4"),
    ("BARON", v"0.2.0"),
    ("BDF", v"0.2.1"),
    ("BEncode", v"0.2.0"),
    ("BGZFStreams", v"0.1.4"),
    ("BIGUQ", v"0.3.3"),
    ("BOSSArrays", v"0.2.2"),
    ("BSplines", v"0.0.3"),
    ("BackpropNeuralNet", v"0.2.0"),
    ("BandedMatrices", v"0.2.3"),
    ("BaseTestDeprecated", v"0.1.0"),
    ("BaseTestNext", v"0.2.2"),
    ("BasisMatrices", v"0.4.1"),
    ("BayesNets", v"1.0.5"),
    ("BayesianDataFusion", v"1.2.0"),
    ("BayesianNonparametrics", v"0.0.1"),
    ("BeaData", v"0.1.0"),
    ("Bebop", v"0.0.1"),
    ("Benchmark", v"0.1.0"),
    ("BenchmarkLite", v"0.1.2"),
    ("BenchmarkProfiles", v"0.0.2"),
    ("BenchmarkTools", v"0.0.8"),
    ("Bezier", v"0.0.1"),
    ("BigArrays", v"0.5.3"),
    ("BinDeps", v"0.7.0"),
    ("Bio", v"0.4.7"),
    ("BioAlignments", v"0.1.0"),
    ("BioArgParse", v"0.0.2"),
    ("BioCore", v"1.2.1"),
    ("BioEnergeticFoodWebs", v"0.3.1"),
    ("BioMedQuery", v"0.2.2"),
    ("BioSeq", v"0.4.0"),
    ("BioSequences", v"0.6.3"),
    ("BioSymbols", v"1.2.0"),
    ("BiomolecularStructures", v"0.2.3"),
    ("Biryani", v"0.2.0"),
    ("BlackBoxOptim", v"0.3.0"),
    ("Blink", v"0.5.4"),
    ("BlockArrays", v"0.1.1"),
    ("Blocks", v"0.1.0"),
    ("BloomFilters", v"0.2.0"),
    ("Blosc", v"0.3.0"),
    ("BlossomV", v"0.3.0"),
    ("BlsData", v"0.0.2"),
    ("Bokeh", v"0.2.0"),
    ("Boltzmann", v"0.5.0"),
    ("Bootstrap", v"1.1.0"),
    ("BoundingBoxes", v"0.2.0"),
    ("Box0", v"0.2.0"),
    ("Brim", v"1.2.2"),
    ("Brownian", v"0.2.0"),
    ("BufferedStreams", v"0.3.3"),
    ("BuildExecutable", v"0.1.2"),
    ("Bukdu", v"0.1.0"),
    ("BusinessDays", v"0.7.3"),
    ("CBOR", v"0.0.2"),
    ("CDDLib", v"0.1.4"),
    ("CLBLAS", v"1.1.5"),
    ("CLFFT", v"0.5.2"),
    ("CMakeWrapper", v"0.0.2"),
    ("COBRA", v"0.1.2"),
    ("COESA", v"0.0.1"),
    ("COFF", v"0.0.2"),
    ("COMTRADE", v"0.1.0"),
    ("CPLEX", v"0.2.8"),
    ("CPUTime", v"0.0.5"),
    ("CQL", v"0.0.1"),
    ("CRC", v"2.0.0"),
    ("CRC32", v"0.0.2"),
    ("CRF", v"0.1.1"),
    ("CRlibm", v"0.5.0"),
    ("CSDP", v"0.0.3"),
    ("CSTParser", v"0.2.0"),
    ("CSV", v"0.1.5"),
    ("CSVFiles", v"0.1.1"),
    ("CUBLAS", v"0.1.0"),
    ("CUDA", v"0.3.0"),
    ("CUDAapi", v"0.1.3"),
    ("CUDAdrv", v"0.5.4"),
    ("CUDAnative", v"0.3.0"),
    ("CUDAnativelib", v"0.0.1"),
    ("CUDArt", v"0.4.1"),
    ("CUDNN", v"0.3.0"),
    ("CUFFT", v"0.0.4"),
    ("CURAND", v"0.0.4"),
    ("CUSOLVER", v"0.2.0"),
    ("CUSPARSE", v"0.6.0"),
    ("CUTEst", v"0.2.0"),
    ("CVXOPT", v"0.2.0"),
    ("Caesar", v"0.0.5"),
    ("Cairo", v"0.4.0"),
    ("Calc", v"0.1.1"),
    ("Calculus", v"0.2.2"),
    ("Calendar", v"0.4.3"),
    ("Cartesian", v"0.3.0"),
    ("CasaCore", v"0.1.0"),
    ("Cascadia", v"0.2.0"),
    ("CatIndices", v"0.0.2"),
    ("CatViews", v"0.1.0"),
    ("Catalan", v"0.0.3"),
    ("CategoricalArrays", v"0.1.6"),
    ("CauseMap", v"0.0.3"),
    ("Cbc", v"0.3.2"),
    ("Celeste", v"0.3.0"),
    ("CellularAutomata", v"0.1.2"),
    ("ChainMap", v"0.1.3"),
    ("ChainRecursive", v"0.0.4"),
    ("ChainedVectors", v"0.0.0"),
    ("Changepoints", v"0.2.0"),
    ("ChaosCommunications", v"0.0.1"),
    ("Checkers", v"0.0.1"),
    ("Chemfiles", v"0.7.4"),
    ("ChemicalKinetics", v"0.2.0"),
    ("Chipmunk", v"0.0.5"),
    ("ChunkedArrays", v"0.1.1"),
    ("Church", v"0.0.1"),
    ("CirruParser", v"0.0.2"),
    ("Clang", v"0.2.0"),
    ("ClassicalCiphers", v"0.3.1"),
    ("Cliffords", v"0.4.0"),
    ("ClimateTools", v"0.1.2"),
    ("ClinicalTrialSampleSize", v"0.0.3"),
    ("Clipper", v"0.3.0"),
    ("ClobberingReload", v"0.2.1"),
    ("Clockwork", v"0.2.0"),
    ("CloudGraphs", v"0.0.2"),
    ("Clp", v"0.3.1"),
    ("ClusterDicts", v"0.1.0"),
    ("ClusterManagers", v"0.2.0"),
    ("ClusterUtils", v"0.0.3"),
    ("Clustering", v"0.9.1"),
    ("CmplxRoots", v"0.0.2"),
    ("CodeTools", v"0.4.7"),
    ("CodecBzip2", v"0.2.2"),
    ("CodecXz", v"0.2.0"),
    ("CodecZlib", v"0.2.0"),
    ("CodecZstd", v"0.2.0"),
    ("Codecs", v"0.4.0"),
    ("CoinOptServices", v"0.2.0"),
    ("CollisionDetection", v"0.0.1"),
    ("Color", v"0.4.8"),
    ("ColorBrewer", v"0.3.1"),
    ("ColorSchemes", v"1.3.0"),
    ("ColorTypes", v"0.5.2"),
    ("ColorVectorSpace", v"0.4.4"),
    ("Colors", v"0.7.4"),
    ("Combinatorics", v"0.5.0"),
    ("CommonCrawl", v"0.0.1"),
    ("CommonSubexpressions", v"0.0.1"),
    ("CompEcon", v"0.3.1"),
    ("CompScienceMeshes", v"0.0.1"),
    ("Compat", v"0.43.0"),
    ("CompilerOptions", v"0.1.0"),
    ("CompilerTools", v"0.2.1"),
    ("Complementarity", v"0.2.3"),
    ("Compose", v"0.5.5"),
    ("ComposeDiff", v"0.0.2"),
    ("CompressedSensing", v"0.0.2"),
    ("ComputationalResources", v"0.1.0"),
    ("ConcaveHull", v"0.0.2"),
    ("Conda", v"0.7.1"),
    ("ConfParser", v"0.0.9"),
    ("ConfidenceWeighted", v"0.0.2"),
    ("ConicBenchmarkUtilities", v"0.2.3"),
    ("ConicIP", v"0.0.1"),
    ("ConicNonlinearBridge", v"0.1.3"),
    ("ConjugatePriors", v"0.1.2"),
    ("ConnectSDK", v"0.1.5"),
    ("ConnectionPools", v"0.0.2"),
    ("ConstructiveSolidGeometry", v"0.1.0"),
    ("ContMechTensors", v"0.3.0"),
    ("ContinuedFractions", v"0.1.0"),
    ("Contour", v"0.3.0"),
    ("ControlCore", v"0.0.1"),
    ("ControlSystems", v"0.3.0"),
    ("ControlToolbox", v"0.1.0"),
    ("Convertible", v"0.1.3"),
    ("Convex", v"0.6.0"),
    ("ConvexSwitch", v"0.0.1"),
    ("CoordinateDescent", v"0.0.1"),
    ("CoordinateTransformations", v"0.4.1"),
    ("CoreNLP", v"0.1.0"),
    ("Cosmology", v"0.3.0"),
    ("Counters", v"0.0.3"),
    ("CoupledFields", v"0.0.1"),
    ("CovarianceMatrices", v"0.3.1"),
    ("Coverage", v"0.4.0"),
    ("CoverageBase", v"0.0.3"),
    ("Cpp", v"0.1.0"),
    ("CpuId", v"0.1.10"),
    ("Crayons", v"0.4.0"),
    ("CreateMacrosFrom", v"0.0.1"),
    ("CrossDecomposition", v"0.0.1"),
    ("CrossfilterCharts", v"0.1.1"),
    ("Crypto", v"0.0.1"),
    ("Cuba", v"0.4.0"),
    ("Cubature", v"1.2.2"),
    ("Cumulants", v"0.2.1"),
    ("Curl", v"0.0.3"),
    ("Currencies", v"0.9.0"),
    ("CurveFit", v"0.1.1"),
    ("CustomUnitRanges", v"0.0.4"),
    ("CutPruners", v"0.0.2"),
    ("Cxx", v"0.1.1"),
    ("CxxWrap", v"0.5.0"),
    ("DASKR", v"0.5.0"),
    ("DASSL", v"0.3.0"),
    ("DBAPI", v"0.1.0"),
    ("DCEMRI", v"0.2.0"),
    ("DICOM", v"0.0.1"),
    ("DReal", v"0.1.0"),
    ("DSGE", v"0.3.1"),
    ("DSP", v"0.3.4"),
    ("DWARF", v"0.1.0"),
    ("Dagger", v"0.2.2"),
    ("DataArrays", v"0.5.3"),
    ("DataAssim", v"0.0.2"),
    ("DataCubes", v"0.1.0"),
    ("DataFeatures", v"0.0.4"),
    ("DataFlow", v"0.2.0"),
    ("DataFrameUtils", v"0.0.1"),
    ("DataFrames", v"0.10.1"),
    ("DataFramesMeta", v"0.2.0"),
    ("DataStreams", v"0.1.3"),
    ("DataStreamsIntegrationTests", v"0.0.2"),
    ("DataStructures", v"0.6.1"),
    ("DataTables", v"0.0.3"),
    ("DataValueArrays", v"0.0.1"),
    ("DataValues", v"0.1.1"),
    ("DateParser", v"0.1.1"),
    ("DateOffsets", v"0.1.2"),
    ("Dates", v"0.4.4"),
    ("Datetime", v"0.1.7"),
    ("Debug", v"0.1.6"),
    ("DecFP", v"0.3.0"),
    ("Decimals", v"0.1.0"),
    ("DecisionTree", v"0.6.5"),
    ("DeclarativePackages", v"0.1.2"),
    ("Deconvolution", v"0.0.1"),
    ("DeepDiffs", v"1.0.1"),
    ("Defer", v"0.0.1"),
    ("DeferredFutures", v"0.5.0"),
    ("DelaunayMeshes", v"0.1.0"),
    ("DelayDiffEq", v"0.4.0"),
    ("Deldir", v"0.0.3"),
    ("Dendriform", v"0.0.3"),
    ("DependentBootstrap", v"0.0.1"),
    ("DetectionTheory", v"0.1.0"),
    ("DeterminantalPointProcesses", v"0.0.1"),
    ("DeterministicPolicyGradient", v"0.0.1"),
    ("DevIL", v"0.4.2"),
    ("Devectorize", v"0.4.2"),
    ("DictFiles", v"0.1.0"),
    ("DictUtils", v"0.0.2"),
    ("Dierckx", v"0.3.0"),
    ("DiffBase", v"0.2.0"),
    ("DiffEqBase", v"1.13.0"),
    ("DiffEqBiological", v"0.3.0"),
    ("DiffEqCallbacks", v"0.2.1"),
    ("DiffEqDevTools", v"0.9.5"),
    ("DiffEqDiffTools", v"0.0.1"),
    ("DiffEqFinancial", v"0.2.0"),
    ("DiffEqJump", v"0.4.0"),
    ("DiffEqMonteCarlo", v"0.7.0"),
    ("DiffEqNoiseProcess", v"0.2.0"),
    ("DiffEqPDEBase", v"0.3.5"),
    ("DiffEqParamEstim", v"0.5.0"),
    ("DiffEqProblemLibrary", v"0.9.1"),
    ("DiffEqSensitivity", v"0.2.2"),
    ("DiffEqUncertainty", v"0.1.0"),
    ("DifferentialDynamicProgramming", v"0.2.0"),
    ("DifferentialEquations", v"2.2.1"),
    ("Digits", v"0.1.4"),
    ("DimensionalPlotRecipes", v"0.0.2"),
    ("DimensionalityReduction", v"0.1.2"),
    ("DirichletProcessMixtures", v"0.0.1"),
    ("DiscreteFactor", v"0.0.0"),
    ("Discretizers", v"1.1.0"),
    ("DiscriminantAnalysis", v"0.1.0"),
    ("Dispatcher", v"0.3.1"),
    ("Displaz", v"0.0.1"),
    ("DistQuads", v"0.1.0"),
    ("Distance", v"0.5.1"),
    ("Distances", v"0.4.1"),
    ("DistributedArrays", v"0.3.0"),
    ("Distributions", v"0.13.0"),
    ("Divergences", v"0.1.0"),
    ("Diversity", v"0.3.1"),
    ("DocOpt", v"0.2.1"),
    ("DocStringExtensions", v"0.3.4"),
    ("Docile", v"0.5.23"),
    ("Docker", v"0.0.1"),
    ("DocumentFunction", v"0.1.5"),
    ("Documenter", v"0.11.4"),
    ("Dolang", v"0.1.2"),
    ("Dolo", v"0.2.0"),
    ("Dopri", v"0.4.0"),
    ("DoubleDouble", v"0.2.1"),
    ("DrakeVisualizer", v"0.1.3"),
    ("Drawing", v"0.1.5"),
    ("DriftDiffusionPoissonSystems", v"0.1.0"),
    ("Dtree", v"0.0.1"),
    ("DualNumbers", v"0.3.0"),
    ("DustExtinction", v"0.2.1"),
    ("DynMultiply", v"0.0.1"),
    ("DynamicDiscreteModels", v"0.0.3"),
    ("DynamicMovementPrimitives", v"0.2.0"),
    ("DynamicalBilliards", v"1.2.1"),
    ("DynamicalBilliardsPlotting", v"0.1.0"),
    ("DynamoDB", v"0.0.1"),
    ("Dynare", v"0.0.1"),
    ("ECOS", v"0.7.2"),
    ("EEG", v"0.2.0"),
    ("EGM96", v"0.0.1"),
    ("ELF", v"0.1.0"),
    ("EMIRT", v"0.5.0"),
    ("ERFA", v"0.2.2"),
    ("EasyPhys", v"0.2.0"),
    ("EasyPkg", v"0.3.0"),
    ("EcologicalNetwork", v"1.0.1"),
    ("EconDatasets", v"0.0.2"),
    ("EconModels", v"0.0.1"),
    ("EgyptianFractions", v"0.0.1"),
    ("Einsum", v"0.1.0"),
    ("ElasticFDA", v"0.4.1"),
    ("ElectricityMarkets", v"0.2.0"),
    ("Elemental", v"0.1.2"),
    ("EllipsisNotation", v"0.2.2"),
    ("Elliptic", v"0.3.0"),
    ("EllipticFEM", v"0.0.2"),
    ("Elly", v"0.0.9"),
    ("EmpiricalRisks", v"0.2.4"),
    ("EndpointRanges", v"0.0.2"),
    ("EnglishText", v"0.4.0"),
    ("EnhancedGJK", v"0.0.1"),
    ("Ensemble", v"0.0.1"),
    ("Equations", v"0.3.0"),
    ("Erdos", v"0.5.0"),
    ("ErrorfreeArithmetic", v"0.0.3"),
    ("EscapeString", v"0.0.1"),
    ("Escher", v"0.3.3"),
    ("Espresso", v"0.1.0"),
    ("Etcd", v"0.1.0"),
    ("EventHistory", v"0.1.0"),
    ("Evolutionary", v"0.1.2"),
    ("EvolvingGraphs", v"0.1.0"),
    ("Example", v"0.4.1"),
    ("ExcelFiles", v"0.1.0"),
    ("ExcelReaders", v"0.8.0"),
    ("Expect", v"0.2.3"),
    ("ExperimentalAnalysis", v"0.1.1"),
    ("Expokit", v"0.0.1"),
    ("ExpressionPatterns", v"0.5.0"),
    ("ExpressionUtils", v"0.1.0"),
    ("Extern", v"0.0.2"),
    ("ExtractMacro", v"0.2.0"),
    ("ExtremelyRandomizedTrees", v"0.1.0"),
    ("EzXML", v"0.4.5"),
    ("FEHM", v"0.1.0"),
    ("FFTViews", v"0.0.2"),
    ("FFTW", v"0.0.2"),
    ("FITSIO", v"0.11.0"),
    ("FLAC", v"0.2.2"),
    ("FLANN", v"0.0.4"),
    ("FMIndexes", v"0.1.0"),
    ("FNVHash", v"0.1.0"),
    ("FPTControl", v"0.0.1"),
    ("FTPClient", v"0.2.1"),
    ("FaSTLMM", v"0.0.2"),
    ("FaceDatasets", v"0.2.0"),
    ("FactCheck", v"0.4.3"),
    ("FactorModels", v"0.0.2"),
    ("Faker", v"0.0.3"),
    ("FastAnonymous", v"0.3.3"),
    ("FastArrayOps", v"0.1.0"),
    ("FastClosures", v"0.1.0"),
    ("FastCombinations", v"0.1.0"),
    ("FastGaussQuadrature", v"0.2.1"),
    ("FastRounding", v"0.0.5"),
    ("FastTransforms", v"0.2.2"),
    ("FastaIO", v"0.3.0"),
    ("Fatou", v"0.1.3"),
    ("Feather", v"0.2.5"),
    ("FeatherFiles", v"0.0.2"),
    ("FileFind", v"0.0.0"),
    ("FileIO", v"0.5.2"),
    ("FilePaths", v"0.4.0"),
    ("FinancialMarkets", v"0.1.1"),
    ("FinancialToolbox", v"0.1.3"),
    ("FiniteElementDiffEq", v"0.4.0"),
    ("FiniteStateMachine", v"0.1.1"),
    ("Fire", v"0.0.2"),
    ("FixedEffectModels", v"0.4.0"),
    ("FixedPoint", v"0.0.1"),
    ("FixedPointDecimals", v"0.1.0"),
    ("FixedPointNumbers", v"0.3.9"),
    ("FixedSizeArrays", v"0.2.5"),
    ("FixedSizeDictionaries", v"0.0.1"),
    ("FixedSizeStrings", v"0.0.1"),
    ("Fixtures", v"0.0.2"),
    ("FlatBuffers", v"0.2.0"),
    ("FlexibleArrays", v"1.1.0"),
    ("Flux", v"0.2.1"),
    ("Fontconfig", v"0.1.1"),
    ("ForecastEval", v"0.0.1"),
    ("Formatting", v"0.2.1"),
    ("FortranFiles", v"0.3.0"),
    ("ForwardDiff", v"0.5.0"),
    ("FredData", v"0.2.0"),
    ("FreeType", v"1.3.0"),
    ("FreeTypeAbstraction", v"0.1.0"),
    ("FreqTables", v"0.1.0"),
    ("FrontierEfficiencyAnalysis", v"0.0.1"),
    ("FunctionWrappers", v"0.1.0"),
    ("FunctionalCollections", v"0.3.2"),
    ("FunctionalData", v"0.1.2"),
    ("FunctionalDataUtils", v"0.1.0"),
    ("FunctionalUtils", v"0.0.0"),
    ("FusionDirect", v"0.2.3"),
    ("GARCH", v"0.3.0"),
    ("GLAbstraction", v"0.3.2"),
    ("GLBooks", v"0.0.1"),
    ("GLFW", v"1.5.0"),
    ("GLM", v"0.7.0"),
    ("GLMNet", v"0.2.0"),
    ("GLPK", v"0.4.2"),
    ("GLPKMathProgInterface", v"0.3.4"),
    ("GLPlot", v"0.0.5"),
    ("GLText", v"0.0.4"),
    ("GLUT", v"0.4.0"),
    ("GLVisualize", v"0.3.0"),
    ("GLWindow", v"0.3.3"),
    ("GMT", v"0.2.0"),
    ("GPUArrays", v"0.0.2"),
    ("GR", v"0.26.0"),
    ("GSDicts", v"0.5.2"),
    ("GSL", v"0.3.6"),
    ("GUITestRunner", v"0.0.2"),
    ("GZip", v"0.3.0"),
    ("Gadfly", v"0.6.5"),
    ("GadflyDiff", v"0.0.1"),
    ("Gallium", v"0.0.4"),
    ("GasModels", v"0.1.0"),
    ("Gasp", v"0.1.0"),
    ("Gaston", v"0.6.0"),
    ("GaussQuadrature", v"0.4.0"),
    ("GaussianMixtureTest", v"0.0.4"),
    ("GaussianMixtures", v"0.1.0"),
    ("GaussianProcesses", v"0.4.0"),
    ("GeneralizedMetropolisHastings", v"0.2.1"),
    ("GeneralizedSampling", v"0.0.6"),
    ("GeneralizedSchurAlgorithm", v"0.0.2"),
    ("GenericSVD", v"0.1.0"),
    ("GeneticAlgorithms", v"0.0.3"),
    ("GeneticVariation", v"0.1.0"),
    ("GenomicFeatures", v"0.2.1"),
    ("GenomicVectors", v"0.1.1"),
    ("GeoEfficiency", v"0.8.7"),
    ("GeoIP", v"0.2.2"),
    ("GeoInterface", v"0.2.1"),
    ("GeoJSON", v"0.2.0"),
    ("GeoStats", v"0.2.1"),
    ("GeoStatsImages", v"0.1.2"),
    ("Geodesy", v"0.3.1"),
    ("GeographicLibPy", v"0.0.3"),
    ("GeometricalPredicates", v"0.0.6"),
    ("GeometryTypes", v"0.4.0"),
    ("GeostatInversion", v"0.2.7"),
    ("GetC", v"1.1.1"),
    ("Gettext", v"0.1.0"),
    ("GibbsSeaWater", v"0.0.4"),
    ("Gillespie", v"0.0.2"),
    ("Git", v"0.1.0"),
    ("GitHub", v"2.2.0"),
    ("GitLab", v"0.0.2"),
    ("Glob", v"1.1.1"),
    ("GnuTLS", v"0.0.5"),
    ("GoogleCharts", v"0.0.7"),
    ("GoogleCloud", v"0.0.4"),
    ("GracePlot", v"0.2.0"),
    ("GradientBoost", v"0.0.1"),
    ("Graft", v"0.0.2"),
    ("GraphCentrality", v"0.1.0"),
    ("GraphGLRM", v"0.2.1"),
    ("GraphIO", v"0.0.1"),
    ("GraphLayout", v"0.3.1"),
    ("GraphMatrices", v"0.1.0"),
    ("GraphPlot", v"0.1.0"),
    ("GraphViz", v"0.1.1"),
    ("Graphics", v"0.2.0"),
    ("Graphs", v"0.8.0"),
    ("GreatCircle", v"0.1.0"),
    ("Grid", v"0.4.2"),
    ("GridInterpolations", v"0.0.2"),
    ("GroupSlices", v"0.0.1"),
    ("GroveAlg", v"0.0.2"),
    ("GrowableArrays", v"0.0.5"),
    ("GslibIO", v"0.0.1"),
    ("Gtk", v"0.13.1"),
    ("GtkBuilderAid", v"0.1.0"),
    ("GtkReactive", v"0.2.3"),
    ("GtkUtilities", v"0.2.3"),
    ("Gumbo", v"0.3.0"),
    ("Gunrock", v"0.0.2"),
    ("Gurobi", v"0.3.3"),
    ("H5SectionsArrays", v"0.0.1"),
    ("HDF5", v"0.8.8"),
    ("HDFS", v"0.1.0"),
    ("HPAT", v"0.0.4"),
    ("HPack", v"0.1.0"),
    ("HSA", v"0.0.1"),
    ("HTSLIB", v"0.0.2"),
    ("HTTP", v"0.4.3"),
    ("HTTP2", v"0.1.0"),
    ("HTTPClient", v"0.2.1"),
    ("Hadamard", v"0.2.2"),
    ("HarwellRutherfordBoeing", v"0.0.1"),
    ("Hecke", v"0.2.1"),
    ("Helpme", v"0.0.13"),
    ("Hermetic", v"0.1.0"),
    ("HexEdit", v"0.0.6"),
    ("Hexagons", v"0.1.0"),
    ("Hiccup", v"0.1.1"),
    ("HiddenMarkovModels", v"0.0.2"),
    ("HierarchicalMatrices", v"0.0.2"),
    ("Highlights", v"0.2.1"),
    ("Hinton", v"0.1.1"),
    ("Hive", v"0.0.3"),
    ("Homebrew", v"0.5.9"),
    ("HopfieldNets", v"0.0.0"),
    ("HttpCommon", v"0.3.0"),
    ("HttpParser", v"0.3.1"),
    ("HttpServer", v"0.2.0"),
    ("Humanize", v"0.4.1"),
    ("Hungarian", v"0.0.4"),
    ("Hwloc", v"0.6.2"),
    ("HyperDualNumbers", v"1.1.1"),
    ("HyperLogLog", v"0.0.0"),
    ("HypothesisTests", v"0.5.1"),
    ("ICOADSDict", v"0.0.1"),
    ("ICU", v"0.4.4"),
    ("IDRsSolver", v"0.1.3"),
    ("IDXParser", v"0.1.0"),
    ("IJulia", v"1.7.0"),
    ("IJuliaPortrayals", v"0.0.4"),
    ("IOIndents", v"0.1.0"),
    ("IPNets", v"0.3.0"),
    ("IPPCore", v"0.2.1"),
    ("IPPDSP", v"0.0.1"),
    ("IProfile", v"0.4.0"),
    ("ISPC", v"0.0.1"),
    ("IdentityRanges", v"0.0.1"),
    ("ImageAxes", v"0.3.1"),
    ("ImageCore", v"0.3.3"),
    ("ImageFiltering", v"0.1.4"),
    ("ImageInTerminal", v"0.2.1"),
    ("ImageMagick", v"0.4.0"),
    ("ImageMetadata", v"0.2.3"),
    ("ImageProjectiveGeometry", v"0.2.0"),
    ("ImageQuilting", v"0.4.6"),
    ("ImageRegistration", v"0.0.1"),
    ("ImageTransformations", v"0.3.1"),
    ("ImageView", v"0.5.1"),
    ("Images", v"0.10.1"),
    ("ImagineFormat", v"0.6.0"),
    ("Immerse", v"0.0.13"),
    ("ImmutableArrays", v"0.0.12"),
    ("ImplicitEquations", v"0.3.0"),
    ("ImputeNaNs", v"0.0.2"),
    ("IncGammaBeta", v"0.0.1"),
    ("IncrementalInference", v"0.2.11"),
    ("IndependentRandomSequences", v"0.0.1"),
    ("IndexableBitVectors", v"0.1.2"),
    ("IndexedArrays", v"0.3.0"),
    ("IndexedTables", v"0.1.8"),
    ("Indicators", v"0.2.2"),
    ("IndirectArrays", v"0.1.1"),
    ("InfoZIP", v"0.1.6"),
    ("InformationMeasures", v"0.1.1"),
    ("InformedDifferentialEvolution", v"0.2.0"),
    ("IniFile", v"0.3.1"),
    ("InplaceOps", v"0.1.0"),
    ("InspectDR", v"0.2.3"),
    ("Instruments", v"0.1.0"),
    ("IntArrays", v"0.0.1"),
    ("IntModN", v"0.0.2"),
    ("Interact", v"0.6.4"),
    ("InteractiveFixedEffectModels", v"0.2.1"),
    ("InterestRates", v"0.1.3"),
    ("Interfaces", v"0.0.4"),
    ("Interpolations", v"0.6.3"),
    ("IntervalArithmetic", v"0.11.0"),
    ("IntervalConstraintProgramming", v"0.7.0"),
    ("IntervalContractors", v"0.1.0"),
    ("IntervalLinearEquations", v"0.2.0"),
    ("IntervalOptimisation", v"0.1.0"),
    ("IntervalRootFinding", v"0.1.1"),
    ("IntervalSets", v"0.1.1"),
    ("IntervalTrees", v"0.3.0"),
    ("IntervalWavelets", v"0.0.4"),
    ("Ipopt", v"0.2.6"),
    ("Isotonic", v"0.0.1"),
    ("IterTools", v"0.1.0"),
    ("IterableTables", v"0.4.2"),
    ("IterationManagers", v"0.0.1"),
    ("IterativeSolvers", v"0.3.1"),
    ("Iterators", v"0.3.1"),
    ("Ito", v"0.0.2"),
    ("JACKAudio", v"0.1.1"),
    ("JDBC", v"0.2.1"),
    ("JFVM", v"1.0.1"),
    ("JLD", v"0.6.11"),
    ("JLDArchives", v"0.1.0"),
    ("JPLEphemeris", v"0.4.1"),
    ("JSON", v"0.14.0"),
    ("JWAS", v"0.2.0"),
    ("Jackknife", v"0.2.0"),
    ("Jacobi", v"0.3.0"),
    ("Jags", v"1.0.6"),
    ("JavaCall", v"0.5.2"),
    ("JellyFish", v"0.0.1"),
    ("Jewel", v"1.0.8"),
    ("JointMoments", v"0.2.5"),
    ("JuLIP", v"0.1.1"),
    ("JuMP", v"0.17.1"),
    ("JuMPChance", v"0.4.0"),
    ("JuMPeR", v"0.5.0"),
    ("JudyDicts", v"0.0.0"),
    ("JuliaDB", v"0.1.2"),
    ("JuliaFEM", v"0.2.1"),
    ("JuliaParser", v"0.7.4"),
    ("JuliaWebAPI", v"0.3.6"),
    ("JuliaWebRepl", v"0.0.0"),
    ("JulieTest", v"0.0.2"),
    ("Julz", v"0.0.2"),
    ("Jumos", v"0.2.1"),
    ("Juniper", v"0.1.1"),
    ("Juno", v"0.3.2"),
    ("KCores", v"0.0.3"),
    ("KDTrees", v"0.6.0"),
    ("KLDivergence", v"0.0.0"),
    ("KNITRO", v"0.3.1"),
    ("KSVD", v"0.0.1"),
    ("KShiftsClustering", v"0.1.0"),
    ("Kafka", v"0.1.0"),
    ("KernSmooth", v"0.0.3"),
    ("KernelDensity", v"0.3.2"),
    ("KernelDensityEstimate", v"0.2.6"),
    ("KernelEstimator", v"0.2.2"),
    ("Klara", v"0.8.6"),
    ("Knet", v"0.8.5"),
    ("Kpax3", v"0.1.0"),
    ("Kriging", v"0.1.2"),
    ("KrylovMethods", v"0.4.0"),
    ("LARS", v"0.0.3"),
    ("LCA", v"0.0.1"),
    ("LCIO", v"0.8.0"),
    ("LCMCore", v"0.0.4"),
    ("LCMGL", v"0.0.2"),
    ("LIBLINEAR", v"0.3.0"),
    ("LIBSVM", v"0.1.1"),
    ("LLLplus", v"0.1.1"),
    ("LLVM", v"0.3.11"),
    ("LMDB", v"0.0.4"),
    ("LNR", v"0.0.2"),
    ("LPVSpectral", v"0.0.4"),
    ("LRSLib", v"0.0.6"),
    ("LRUCache", v"0.0.1"),
    ("LSODA", v"0.1.0"),
    ("LTISystems", v"0.1.0"),
    ("LaTeX", v"0.1.1"),
    ("LaTeXStrings", v"0.2.1"),
    ("LambertW", v"0.2.0"),
    ("LanguageServer", v"0.2.2"),
    ("Languages", v"0.2.0"),
    ("Laplacians", v"0.1.4"),
    ("Lasso", v"0.1.0"),
    ("LaxZonedDateTimes", v"0.0.4"),
    ("Lazy", v"0.11.7"),
    ("LazyCall", v"0.0.3"),
    ("LazyContext", v"0.0.2"),
    ("LazyQuery", v"0.0.1"),
    ("LazySequences", v"0.1.0"),
    ("LearnBase", v"0.1.6"),
    ("LearningStrategies", v"0.0.1"),
    ("LeastSquaresOptim", v"0.4.0"),
    ("LegacyStrings", v"0.3.0"),
    ("Lens", v"0.0.3"),
    ("LevelDB", v"1.1.3"),
    ("Levenshtein", v"0.0.2"),
    ("Lexicon", v"0.1.18"),
    ("LibArchive", v"0.0.2"),
    ("LibBSON", v"0.3.0"),
    ("LibCURL", v"0.2.2"),
    ("LibCloud", v"0.0.2"),
    ("LibExpat", v"0.3.1"),
    ("LibGEOS", v"0.2.1"),
    ("LibGit2", v"0.3.8"),
    ("LibHealpix", v"0.2.4"),
    ("LibPSF", v"0.2.0"),
    ("LibSndFile", v"1.0.0"),
    ("LibTrading", v"0.0.1"),
    ("Libz", v"0.2.4"),
    ("LifeTable", v"0.1.0"),
    ("LightGraphs", v"0.10.2"),
    ("LightGraphsExtras", v"0.1.0"),
    ("LightXML", v"0.5.0"),
    ("LineEdit", v"0.0.1"),
    ("LineSearches", v"2.2.1"),
    ("LinearLeastSquares", v"0.2.0"),
    ("LinearMaps", v"0.3.0"),
    ("LinearOperators", v"0.3.1"),
    ("LinearResponseVariationalBayes", v"0.0.1"),
    ("LinguisticData", v"0.0.2"),
    ("Lint", v"0.5.2"),
    ("LispSyntax", v"0.1.5"),
    ("LittleEndianBase128", v"0.0.1"),
    ("Loess", v"0.3.0"),
    ("LogParser", v"0.3.0"),
    ("LoggedDicts", v"0.0.2"),
    ("Logging", v"0.3.1"),
    ("LombScargle", v"0.3.0"),
    ("Lora", v"0.5.6"),
    ("Loss", v"0.0.1"),
    ("LossFunctions", v"0.2.0"),
    ("LowDimNearestNeighbors", v"0.0.1"),
    ("LowRankApprox", v"0.0.2"),
    ("LowRankModels", v"0.2.0"),
    ("LsqFit", v"0.3.0"),
    ("Lumberjack", v"2.1.1"),
    ("Lumira", v"0.0.2"),
    ("Luxor", v"0.9.3"),
    ("MAT", v"0.4.0"),
    ("MATLAB", v"0.5.1"),
    ("MATLABCluster", v"0.0.1"),
    ("MCMC", v"0.3.0"),
    ("MDCT", v"1.0.1"),
    ("MDPs", v"0.1.1"),
    ("MFCC", v"0.1.1"),
    ("MIDI", v"0.1.0"),
    ("MINPACK", v"0.1.0"),
    ("MIToS", v"2.0.0"),
    ("MKLSparse", v"0.1.0"),
    ("MLBase", v"0.6.1"),
    ("MLDataPattern", v"0.1.2"),
    ("MLDataUtils", v"0.2.0"),
    ("MLKernels", v"0.2.0"),
    ("MLLabelUtils", v"0.1.4"),
    ("MNIST", v"0.0.2"),
    ("MP3", v"1.0.0"),
    ("MPFI", v"0.1.0"),
    ("MPI", v"0.5.1"),
    ("MUMPS", v"0.0.2"),
    ("MXNet", v"0.2.3"),
    ("MachO", v"0.0.4"),
    ("MachineLearning", v"0.0.3"),
    ("MachineLearningMetrics", v"0.0.1"),
    ("MacroTools", v"0.3.7"),
    ("Mads", v"0.4.33"),
    ("Maker", v"0.3.1"),
    ("Mamba", v"0.11.1"),
    ("Mandrill", v"0.1.0"),
    ("ManifoldLearning", v"0.1.0"),
    ("MapLight", v"0.0.2"),
    ("MappedArrays", v"0.0.7"),
    ("Maracas", v"0.0.6"),
    ("Markdown", v"0.3.0"),
    ("MarketData", v"0.6.0"),
    ("MarketTechnicals", v"0.5.1"),
    ("Match", v"0.3.0"),
    ("Matcha", v"0.0.2"),
    ("MathProgBase", v"0.6.4"),
    ("MathToolkit", v"0.0.1"),
    ("Mathematica", v"0.2.0"),
    ("MatlabCompat", v"0.1.2"),
    ("MatpowerCases", v"0.5.0"),
    ("MatrixChainMultiply", v"0.0.1"),
    ("MatrixDepot", v"0.5.6"),
    ("MatrixMarket", v"0.2.0"),
    ("MatrixNetworks", v"0.1.0"),
    ("Maxima", v"0.0.4"),
    ("Mayday", v"0.0.3"),
    ("MbedTLS", v"0.4.5"),
    ("MeCab", v"0.1.7"),
    ("Measurements", v"0.4.0"),
    ("Measures", v"0.1.0"),
    ("Meddle", v"0.0.6"),
    ("Media", v"0.3.0"),
    ("MelGeneralizedCepstrums", v"0.0.4"),
    ("Memcache", v"0.1.1"),
    ("Memento", v"0.3.2"),
    ("Memoize", v"0.2.0"),
    ("MergedMethods", v"0.0.3"),
    ("Merly", v"0.0.2"),
    ("MeshIO", v"0.0.6"),
    ("Meshes", v"0.2.0"),
    ("Meshing", v"0.0.3"),
    ("MessageUtils", v"0.0.2"),
    ("MetaProgTools", v"0.2.2"),
    ("MetaTools", v"0.0.1"),
    ("MetadataTools", v"0.3.2"),
    ("Metamath", v"0.1.0"),
    ("Metis", v"0.0.10"),
    ("Microbiome", v"0.1.0"),
    ("Microeconometrics", v"0.1.2"),
    ("Millboard", v"0.1.0"),
    ("Mimi", v"0.3.0"),
    ("MiniLogging", v"0.0.2"),
    ("Miniball", v"0.0.1"),
    ("MinimalPerfectHashes", v"0.1.2"),
    ("MixedModels", v"0.10.0"),
    ("MixtureModels", v"0.2.0"),
    ("Mocha", v"0.2.0"),
    ("Mocking", v"0.3.4"),
    ("ModernGL", v"0.1.1"),
    ("MolecularDataType", v"0.0.2"),
    ("MolecularDynamics", v"0.1.3"),
    ("MolecularPDB", v"0.0.1"),
    ("Monads", v"0.0.0"),
    ("Mongo", v"0.3.0"),
    ("Mongrel2", v"0.0.0"),
    ("Morsel", v"0.0.6"),
    ("Mosek", v"0.6.4"),
    ("MotoServer", v"0.2.1"),
    ("MsgPack", v"0.1.1"),
    ("MsgPackRpcClient", v"0.0.0"),
    ("MsgPackRpcServer", v"0.0.1"),
    ("MuKanren", v"0.1.0"),
    ("MultiNest", v"0.2.0"),
    ("MultiPoly", v"0.1.0"),
    ("MultiScaleArrays", v"0.3.0"),
    ("MultidimensionalTables", v"0.0.3"),
    ("MultipleTesting", v"0.2.3"),
    ("Multirate", v"0.0.2"),
    ("Multisets", v"0.0.2"),
    ("MultivariatePolynomials", v"0.0.2"),
    ("MultivariateStats", v"0.4.0"),
    ("Munkres", v"0.1.1"),
    ("Murmur3", v"0.2.0"),
    ("Mustache", v"0.1.4"),
    ("MutableStrings", v"0.0.0"),
    ("Mux", v"0.2.3"),
    ("MySQL", v"0.2.0"),
    ("NBInclude", v"1.2.0"),
    ("NBTesting", v"0.0.2"),
    ("NEOS", v"0.2.0"),
    ("NFFT", v"0.2.1"),
    ("NHST", v"0.0.2"),
    ("NIDAQ", v"0.2.0"),
    ("NIfTI", v"0.2.0"),
    ("NKLandscapes", v"0.3.0"),
    ("NLPModels", v"0.3.0"),
    ("NLSolversBase", v"2.1.3"),
    ("NLopt", v"0.3.6"),
    ("NLreg", v"0.1.1"),
    ("NLsolve", v"0.12.1"),
    ("NMEA", v"0.0.6"),
    ("NMF", v"0.2.5"),
    ("NOAAData", v"0.0.2"),
    ("NPZ", v"0.2.0"),
    ("NRRD", v"0.3.3"),
    ("NURBS", v"0.0.1"),
    ("NaNMath", v"0.2.6"),
    ("NaiveBayes", v"0.3.1"),
    ("Named", v"0.0.0"),
    ("NamedArrays", v"0.6.1"),
    ("NamedColors", v"0.1.0"),
    ("NamedDimensions", v"0.0.3"),
    ("NamedTuples", v"4.0.0"),
    ("Napier", v"0.0.2"),
    ("NaturalSort", v"0.0.1"),
    ("NearestNeighbors", v"0.3.0"),
    ("Nemo", v"0.6.3"),
    ("Neo4j", v"1.0.0"),
    ("Neovim", v"0.0.2"),
    ("NetCDF", v"0.5.0"),
    ("Netpbm", v"0.2.1"),
    ("Nettle", v"0.3.0"),
    ("NetworkFlows", v"0.0.1"),
    ("NetworkLayout", v"0.1.1"),
    ("NetworkViz", v"0.0.2"),
    ("NeuralynxNCS", v"0.0.1"),
    ("NodeJS", v"0.2.0"),
    ("NonNegLeastSquares", v"0.1.0"),
    ("NormalizeQuantiles", v"0.4.0"),
    ("NoveltyColors", v"0.3.0"),
    ("NullableArrays", v"0.1.2"),
    ("Nulls", v"0.0.2"),
    ("NumFormat", v"0.0.4"),
    ("NumberedLines", v"0.0.3"),
    ("NumericExtensions", v"0.6.2"),
    ("NumericFuns", v"0.2.4"),
    ("NumericIO", v"0.2.0"),
    ("NumericSuffixes", v"0.0.3"),
    ("NumericalIntegration", v"0.0.3"),
    ("OAuth", v"0.4.1"),
    ("OCCA", v"0.0.1"),
    ("ODBC", v"0.5.2"),
    ("ODE", v"0.7.0"),
    ("ODEInterface", v"0.1.5"),
    ("ODEInterfaceDiffEq", v"0.5.1"),
    ("OEIS", v"0.0.2"),
    ("OIFITS", v"0.3.1"),
    ("OOPMacro", v"0.0.2"),
    ("OSC", v"0.1.0"),
    ("OSXNotifier", v"0.0.1"),
    ("ObjFileBase", v"0.0.4"),
    ("Observables", v"0.0.2"),
    ("OdsIO", v"0.2.0"),
    ("OffsetArrays", v"0.3.0"),
    ("Ogg", v"0.0.2"),
    ("OhMyREPL", v"0.2.6"),
    ("OnlineMoments", v"0.1.0"),
    ("OnlineStats", v"0.8.0"),
    ("OpenCL", v"0.6.1"),
    ("OpenDSSDirect", v"0.4.0"),
    ("OpenEphysLoader", v"0.1.6"),
    ("OpenFOAM", v"0.0.1"),
    ("OpenGL", v"2.0.3"),
    ("OpenGene", v"0.1.11"),
    ("OpenSSL", v"0.0.0"),
    ("OpenSecrets", v"0.0.1"),
    ("OpenSlide", v"0.0.1"),
    ("OpenStreetMap", v"0.8.2"),
    ("OptiMimi", v"0.0.2"),
    ("Optim", v"0.9.3"),
    ("OptimBase", v"1.0.0"),
    ("OptimPack", v"0.3.0"),
    ("OptionalData", v"0.1.0"),
    ("Options", v"0.2.6"),
    ("Opus", v"0.0.2"),
    ("Orchestra", v"0.0.5"),
    ("OrdinaryDiffEq", v"2.9.0"),
    ("PAINTER", v"0.4.0"),
    ("PATHSolver", v"0.3.0"),
    ("PDMats", v"0.7.1"),
    ("PDSampler", v"0.0.1"),
    ("PEG", v"0.0.1"),
    ("PEGParser", v"0.1.2"),
    ("PGFPlots", v"1.7.0"),
    ("PGFPlotsX", v"0.1.5"),
    ("PGM", v"0.0.1"),
    ("PLX", v"0.0.5"),
    ("POMDPModels", v"0.1.3"),
    ("POMDPToolbox", v"0.1.2"),
    ("POMDPs", v"0.5.1"),
    ("PTools", v"0.0.0"),
    ("PValueAdjust", v"2.0.0"),
    ("PackageGenerator", v"0.1.2"),
    ("Packing", v"0.1.0"),
    ("PaddedViews", v"0.1.0"),
    ("Pages", v"0.0.7"),
    ("PairwiseListMatrices", v"0.5.1"),
    ("Pajarito", v"0.4.2"),
    ("Pandas", v"0.5.4"),
    ("ParSpMatVec", v"0.0.1"),
    ("ParallelAccelerator", v"0.2.2"),
    ("ParallelDataTransfer", v"0.2.0"),
    ("ParallelSparseMatMul", v"0.0.2"),
    ("ParameterizedFunctions", v"2.2.0"),
    ("Parameters", v"0.7.3"),
    ("Pardiso", v"0.2.0"),
    ("ParserCombinator", v"1.7.11"),
    ("ParticleFilters", v"0.0.3"),
    ("Pastebin", v"0.0.3"),
    ("Patchwork", v"0.4.0"),
    ("PathDistribution", v"0.0.1"),
    ("PatternDispatch", v"0.2.0"),
    ("Pcap", v"0.0.4"),
    ("PdbTool", v"0.1.0"),
    ("Pedigrees", v"0.0.1"),
    ("PenaltyFunctions", v"0.0.2"),
    ("PerceptualColourMaps", v"0.1.2"),
    ("Permutations", v"0.1.3"),
    ("Persist", v"1.0.0"),
    ("PhantomJS", v"0.0.1"),
    ("Phonetics", v"0.1.0"),
    ("Phylo", v"0.1.3"),
    ("PhyloModels", v"0.2.2"),
    ("PhyloNetworks", v"0.5.1"),
    ("PhyloTrees", v"0.7.0"),
    ("Phylogenetics", v"0.0.2"),
    ("Phylogenies", v"0.1.0"),
    ("PhysOcean", v"0.0.1"),
    ("PiCraft", v"0.1.0"),
    ("PicoSAT", v"0.2.0"),
    ("PiecewiseAffineTransforms", v"0.2.0"),
    ("PiecewiseIncreasingRanges", v"0.0.4"),
    ("PiecewiseLinearOpt", v"0.0.2"),
    ("Pipe", v"1.0.0"),
    ("PkgBenchmark", v"0.0.2"),
    ("PkgDev", v"0.1.6"),
    ("PkgTemplates", v"0.0.1"),
    ("PlanOut", v"0.1.0"),
    ("Playground", v"0.3.0"),
    ("PlotRecipes", v"0.2.0"),
    ("PlotThemes", v"0.1.4"),
    ("PlotUtils", v"0.4.4"),
    ("Plotly", v"0.1.1"),
    ("PlotlyJS", v"0.6.5"),
    ("Plots", v"0.12.3"),
    ("PlyIO", v"0.0.3"),
    ("PolarFact", v"0.0.6"),
    ("Poltergeist", v"0.0.4"),
    ("PolyJuMP", v"0.0.1"),
    ("Polyglot", v"0.1.0"),
    ("Polyhedra", v"0.1.5"),
    ("Polynomial", v"0.1.1"),
    ("PolynomialFactors", v"0.0.6"),
    ("PolynomialMatrices", v"0.2.0"),
    ("PolynomialRoots", v"0.1.0"),
    ("PolynomialZeros", v"0.0.1"),
    ("Polynomials", v"0.1.6"),
    ("PooledArrays", v"0.1.1"),
    ("PortAudio", v"1.0.0"),
    ("PortableGameNotation", v"0.0.2"),
    ("PositiveFactorizations", v"0.1.0"),
    ("PotentialFlow", v"0.0.1"),
    ("PowerLaws", v"0.0.3"),
    ("PowerModels", v"0.5.1"),
    ("PowerModelsAnnex", v"0.0.6"),
    ("PowerSeries", v"0.2.0"),
    ("PrettyNumber", v"0.0.1"),
    ("Primes", v"0.2.0"),
    ("PrivateModules", v"0.0.3"),
    ("PrivateMultiplicativeWeights", v"0.0.2"),
    ("ProfileView", v"0.3.0"),
    ("ProgressMeter", v"0.4.0"),
    ("ProgressiveAligner", v"0.4.0"),
    ("Proj4", v"0.1.0"),
    ("ProjectTemplate", v"0.0.1"),
    ("ProjectiveDictionaryPairLearning", v"0.4.3"),
    ("PropertyGraph", v"0.1.0"),
    ("ProteinEnsembles", v"0.1.1"),
    ("ProtoBuf", v"0.4.0"),
    ("ProximalBase", v"0.0.3"),
    ("ProximalOperators", v"0.3.0"),
    ("PtFEM", v"0.0.3"),
    ("PublicSuffix", v"0.0.2"),
    ("Push", v"0.0.1"),
    ("PyAMG", v"0.2.1"),
    ("PyCall", v"1.15.0"),
    ("PyCallJLD", v"0.0.2"),
    ("PyDSTool", v"0.1.2"),
    ("PyLCM", v"0.1.0"),
    ("PyLexYacc", v"0.0.2"),
    ("PyLogging", v"0.0.1"),
    ("PyPlot", v"2.3.2"),
    ("PyProj", v"0.0.1"),
    ("PySide", v"0.0.2"),
    ("PySyntax", v"0.0.3"),
    ("PyTest", v"0.0.2"),
    ("PyX", v"0.0.1"),
    ("Pyramids", v"0.0.1"),
    ("QDXML", v"0.0.1"),
    ("QDates", v"0.0.2"),
    ("QML", v"0.4.3"),
    ("QNaNs", v"0.1.2"),
    ("QPSParser", v"0.1.0"),
    ("QRupdate", v"1.0.1"),
    ("QWTwPlot", v"0.0.2"),
    ("QuDirac", v"0.1.3"),
    ("QuTiP", v"0.0.1"),
    ("QuadGK", v"0.1.3"),
    ("Quandl", v"0.6.1"),
    ("QuantEcon", v"0.13.0"),
    ("QuantumBayesian", v"0.2.0"),
    ("QuantumInfo", v"0.0.2"),
    ("QuantumLab", v"0.0.3"),
    ("QuantumOptics", v"0.4.2"),
    ("QuantumTomography", v"0.0.2"),
    ("QuartzImageIO", v"0.3.1"),
    ("Quaternions", v"0.2.0"),
    ("Query", v"0.6.0"),
    ("Queueing", v"0.1.0"),
    ("QuickCheck", v"0.0.0"),
    ("QuickHull", v"0.1.0"),
    ("QuickShiftClustering", v"0.1.1"),
    ("QuickTypes", v"0.2.0"),
    ("Qwt", v"0.0.1"),
    ("RCall", v"0.7.5"),
    ("RDF", v"0.0.1"),
    ("RData", v"0.1.0"),
    ("RDatasets", v"0.2.0"),
    ("REPL", v"0.0.2"),
    ("REPLCompletions", v"0.0.3"),
    ("RLEVectors", v"0.3.2"),
    ("RNGTest", v"1.2.0"),
    ("ROCAnalysis", v"0.1.0"),
    ("ROOT", v"0.1.0"),
    ("RRRMC", v"1.0.1"),
    ("Rainflow", v"0.1.0"),
    ("RandomCorrelationMatrices", v"0.3.0"),
    ("RandomFerns", v"0.1.0"),
    ("RandomMatrices", v"0.2.1"),
    ("RandomNumbers", v"0.1.0"),
    ("RandomQuantum", v"0.0.1"),
    ("RangeArrays", v"0.2.0"),
    ("Ranges", v"0.0.1"),
    ("RationalFunctions", v"0.1.1"),
    ("Ratios", v"0.1.0"),
    ("RawArray", v"0.0.2"),
    ("RdRand", v"0.0.0"),
    ("React", v"0.1.6"),
    ("Reactive", v"0.6.0"),
    ("ReactiveBasics", v"0.2.1"),
    ("ReadStat", v"0.1.1"),
    ("ReadWriteDlm2", v"0.6.1"),
    ("ReadWriteLocks", v"0.0.2"),
    ("ReadableNumbers", v"0.0.3"),
    ("RealInterface", v"0.0.3"),
    ("RecipesBase", v"0.2.0"),
    ("Recommendation", v"0.0.1"),
    ("RecurrenceAnalysis", v"0.1.0"),
    ("RecursiveArrayTools", v"0.12.4"),
    ("Redis", v"0.1.1"),
    ("Reduce", v"0.2.2"),
    ("Reel", v"0.2.2"),
    ("Reexport", v"0.1.0"),
    ("RegERMs", v"0.0.2"),
    ("RegionTrees", v"0.0.3"),
    ("Regression", v"0.3.0"),
    ("Reinforce", v"0.0.1"),
    ("Remez", v"0.0.1"),
    ("Requests", v"0.5.1"),
    ("RequestsCache", v"0.0.1"),
    ("RequiredKeywords", v"0.0.1"),
    ("Requires", v"0.4.3"),
    ("Resampling", v"0.0.0"),
    ("ResettableStacks", v"0.2.0"),
    ("Restful", v"0.4.0"),
    ("RestrictProlong", v"0.0.1"),
    ("ResultTypes", v"0.2.0"),
    ("Retry", v"0.1.4"),
    ("ReusableFunctions", v"0.2.9"),
    ("Revealables", v"0.0.1"),
    ("ReverseDiff", v"0.1.5"),
    ("ReverseDiffOverload", v"0.0.1"),
    ("ReverseDiffSource", v"0.3.0"),
    ("ReverseDiffSparse", v"0.7.3"),
    ("RiemannComplexNumbers", v"0.0.1"),
    ("Rif", v"0.0.12"),
    ("RigidBodyDynamics", v"0.4.0"),
    ("RigidBodyTreeInspector", v"0.2.0"),
    ("RingArrays", v"0.1.0"),
    ("RingBuffers", v"1.1.2"),
    ("Rmath", v"0.2.0"),
    ("RoME", v"0.0.8"),
    ("RobotOS", v"0.5.0"),
    ("Robotlib", v"0.2.1"),
    ("RobustLeastSquares", v"0.0.1"),
    ("RobustPmap", v"0.2.8"),
    ("RobustShortestPath", v"0.2.3"),
    ("RobustStats", v"0.0.1"),
    ("RollingFunctions", v"0.2.1"),
    ("RomanNumerals", v"0.2.2"),
    ("Roots", v"0.4.3"),
    ("Rotations", v"0.5.0"),
    ("RouletteWheels", v"0.0.6"),
    ("RoundingIntegers", v"0.0.3"),
    ("RowEchelon", v"0.0.2"),
    ("Rsvg", v"0.1.0"),
    ("RudeOil", v"0.1.0"),
    ("RunTests", v"0.0.3"),
    ("S3DB", v"0.1.0"),
    ("S3Dicts", v"0.5.2"),
    ("SALSA", v"0.1.0"),
    ("SCIP", v"0.3.0"),
    ("SCS", v"0.3.3"),
    ("SDE", v"0.3.1"),
    ("SDL", v"0.1.5"),
    ("SDWBA", v"0.0.2"),
    ("SFML", v"0.2.0"),
    ("SGDOptim", v"0.2.2"),
    ("SGP4", v"0.4.0"),
    ("SHA", v"0.3.3"),
    ("SIMD", v"0.2.0"),
    ("SIUnits", v"0.1.0"),
    ("SLHA", v"0.0.1"),
    ("SMTPClient", v"0.1.0"),
    ("SPTK", v"0.2.1"),
    ("SQLite", v"0.4.5"),
    ("SQSChannels", v"0.5.0"),
    ("SVM", v"0.0.1"),
    ("SVMLightLoader", v"0.3.1"),
    ("SVR", v"0.2.8"),
    ("SampledSignals", v"1.1.0"),
    ("Sampling", v"0.0.8"),
    ("SaveREPL", v"0.0.1"),
    ("SchattenNorms", v"0.0.3"),
    ("SchumakerSpline", v"0.0.1"),
    ("ScikitLearn", v"0.3.0"),
    ("ScikitLearnBase", v"0.3.0"),
    ("Seaborn", v"0.2.0"),
    ("SecureSessions", v"0.0.2"),
    ("Seismic", v"0.1.1"),
    ("SemidefiniteModel", v"0.0.1"),
    ("SemidefiniteModels", v"0.0.3"),
    ("SemidefiniteProgramming", v"0.2.0"),
    ("SenseHat", v"0.1.0"),
    ("SeqMaker", v"0.1.1"),
    ("SerialPorts", v"0.1.0"),
    ("Shannon", v"0.3.0"),
    ("ShapeModels", v"0.0.3"),
    ("Shapefile", v"0.2.0"),
    ("Shearlab", v"0.1.3"),
    ("Shoco", v"0.1.0"),
    ("ShowItLikeYouBuildIt", v"0.0.1"),
    ("ShowSet", v"0.1.1"),
    ("Showoff", v"0.1.1"),
    ("Sigma", v"0.0.1"),
    ("SigmoidalProgramming", v"0.0.1"),
    ("SignalView", v"0.0.1"),
    ("SignedDistanceFields", v"0.2.0"),
    ("Silo", v"0.2.0"),
    ("SimJulia", v"0.3.14"),
    ("SimilaritySearch", v"0.1.5"),
    ("SimpleStructs", v"0.0.2"),
    ("SimpleTasks", v"0.0.12"),
    ("SimpleTraits", v"0.5.1"),
    ("Sims", v"0.2.4"),
    ("SingularIntegralEquations", v"0.2.3"),
    ("SingularSpectrumAnalysis", v"0.0.3"),
    ("SkyCoords", v"0.1.1"),
    ("SliceSampler", v"0.0.0"),
    ("SloanDigitalSkySurvey", v"0.1.1"),
    ("Slugify", v"0.1.1"),
    ("Smile", v"0.2.0"),
    ("SmoothingKernels", v"0.0.0"),
    ("SmoothingSplines", v"0.1.2"),
    ("SnFFT", v"0.0.1"),
    ("Snappy", v"0.1.0"),
    ("SnoopCompile", v"0.1.0"),
    ("Sobol", v"0.2.2"),
    ("Sodium", v"0.0.0"),
    ("SoftConfidenceWeighted", v"0.2.0"),
    ("SolarSystemLib", v"0.0.2"),
    ("SolveDSGE", v"0.1.2"),
    ("SortingAlgorithms", v"0.1.1"),
    ("SortingNetworks", v"0.0.1"),
    ("Soundex", v"0.0.0"),
    ("Spark", v"0.2.0"),
    ("Sparklines", v"0.1.2"),
    ("Sparrow", v"0.0.1"),
    ("SparseGrids", v"0.2.0"),
    ("SparseVectors", v"0.4.2"),
    ("SpatialEcology", v"0.1.0"),
    ("SpatialGrids", v"0.1.0"),
    ("SpecialFunctions", v"0.2.0"),
    ("SpecialMatrices", v"0.1.3"),
    ("Spectra", v"0.3.6"),
    ("SpeedDate", v"0.2.2"),
    ("SpiceData", v"0.2.0"),
    ("SpikingNetworks", v"0.0.4"),
    ("StackTraces", v"0.1.1"),
    ("StackedNets", v"0.0.1"),
    ("Stan", v"2.0.1"),
    ("StandardizedMatrices", v"0.2.0"),
    ("StatFiles", v"0.0.2"),
    ("StatPlots", v"0.4.2"),
    ("StatProfilerHTML", v"0.2.0"),
    ("StateSpaceRoutines", v"0.0.1"),
    ("StatefulIterators", v"0.1.0"),
    ("StaticArrays", v"0.6.1"),
    ("Stats", v"0.1.0"),
    ("StatsBase", v"0.17.0"),
    ("StatsFuns", v"0.5.0"),
    ("StatsModels", v"0.0.2"),
    ("StatsUtils", v"0.0.1"),
    ("StatsdClient", v"0.0.2"),
    ("SteadyStateDiffEq", v"0.1.0"),
    ("SteinDiscrepancy", v"0.0.2"),
    ("StochDynamicProgramming", v"0.5.0"),
    ("StochasticDiffEq", v"2.6.0"),
    ("StochasticSearch", v"0.3.0"),
    ("Stochy", v"0.0.2"),
    ("StokesDiffEq", v"0.2.0"),
    ("StrPack", v"0.1.0"),
    ("StreamStats", v"0.0.2"),
    ("StringDistances", v"0.1.1"),
    ("StringEncodings", v"0.2.3"),
    ("StringInterpolation", v"0.0.1"),
    ("StructIO", v"0.1.0"),
    ("StructJuMP", v"0.0.1"),
    ("StructsOfArrays", v"0.0.3"),
    ("StructuredQueries", v"0.0.4"),
    ("Subsequences", v"0.0.1"),
    ("SubsetSelection", v"0.0.3"),
    ("SuffixArrays", v"0.1.0"),
    ("Sugar", v"0.2.0"),
    ("SugarBLAS", v"0.0.4"),
    ("SuiteSparse", v"0.0.1"),
    ("SumOfSquares", v"0.0.1"),
    ("Sundials", v"0.14.3"),
    ("SunlightAPIs", v"0.0.3"),
    ("Suppressor", v"0.0.5"),
    ("SweepOperator", v"0.1.0"),
    ("SwiftObjectStores", v"0.1.1"),
    ("Swifter", v"0.0.8"),
    ("Switch", v"0.0.1"),
    ("SwitchTimeOpt", v"0.1.0"),
    ("SymDict", v"0.1.3"),
    ("SymEngine", v"0.3.0"),
    ("SymPy", v"0.5.4"),
    ("Symata", v"0.2.0"),
    ("SymmetricBitArrays", v"0.0.1"),
    ("SymmetricTensors", v"0.1.1"),
    ("Synchrony", v"0.0.1"),
    ("SynthesisFilters", v"0.0.4"),
    ("SystemImageBuilder", v"0.0.7"),
    ("TOML", v"0.3.0"),
    ("TablesDemo", v"0.0.1"),
    ("Taro", v"0.5.0"),
    ("Tau", v"0.1.1"),
    ("TaylorIntegration", v"0.1.2"),
    ("TaylorSeries", v"0.5.1"),
    ("TeaSeis", v"0.0.5"),
    ("Temporal", v"0.3.2"),
    ("TemporalWeights", v"0.0.1"),
    ("TensorDecompositions", v"0.2.0"),
    ("TensorFlow", v"0.6.7"),
    ("TensorOperations", v"0.5.1"),
    ("TensorToolbox", v"0.0.2"),
    ("Tensors", v"0.7.0"),
    ("TermWin", v"0.0.31"),
    ("TerminalExtensions", v"0.2.0"),
    ("TerminalUI", v"0.0.2"),
    ("Terminals", v"0.0.1"),
    ("TestImages", v"0.2.0"),
    ("TestRunner", v"0.0.3"),
    ("TestSetExtensions", v"1.0.0"),
    ("TexExtensions", v"0.1.0"),
    ("TextAnalysis", v"0.2.1"),
    ("TextModel", v"0.1.3"),
    ("TextParse", v"0.2.0"),
    ("TextPlots", v"0.3.0"),
    ("TextWrap", v"0.2.0"),
    ("ThermodynamicsTable", v"0.0.4"),
    ("ThingSpeak", v"0.0.2"),
    ("ThreeJS", v"0.3.0"),
    ("Thrift", v"0.3.0"),
    ("TikzGraphs", v"0.6.0"),
    ("TikzPictures", v"0.4.0"),
    ("TiledIteration", v"0.0.2"),
    ("TimeData", v"0.7.0"),
    ("TimeFrames", v"0.0.6"),
    ("TimeModels", v"0.0.3"),
    ("TimeSeries", v"0.10.0"),
    ("TimeSeriesIO", v"0.0.4"),
    ("TimeSeriesResampler", v"0.0.6"),
    ("TimeZones", v"0.4.2"),
    ("TimerOutputs", v"0.2.6"),
    ("Timestamps", v"0.0.4"),
    ("TinyInt", v"0.0.1"),
    ("TinySegmenter", v"0.0.4"),
    ("TinySparse", v"0.0.1"),
    ("Tk", v"0.5.2"),
    ("ToeplitzMatrices", v"0.2.0"),
    ("Tokenize", v"0.2.0"),
    ("TopicModels", v"0.0.1"),
    ("TopicModelsVB", v"0.0.1"),
    ("TrafficAssignment", v"0.4.1"),
    ("TranscodingStreams", v"0.2.0"),
    ("TransformUtils", v"0.0.7"),
    ("Transit", v"0.8.1"),
    ("Transpiler", v"0.0.1"),
    ("TravelingSalesmanHeuristics", v"0.1.0"),
    ("TreeView", v"0.1.0"),
    ("Trie", v"0.0.0"),
    ("Turing", v"0.3.0"),
    ("Twiddle", v"0.4.0"),
    ("Twitter", v"0.3.0"),
    ("TwoBasedIndexing", v"0.0.1"),
    ("TypeCheck", v"0.0.3"),
    ("Typeclass", v"0.0.1"),
    ("TypedDelegation", v"0.1.3"),
    ("TypedTables", v"0.2.0"),
    ("UAParser", v"0.4.0"),
    ("ULID", v"0.2.0"),
    ("URIParser", v"0.2.0"),
    ("URITemplate", v"0.0.1"),
    ("URLParse", v"0.0.0"),
    ("UTF16", v"0.3.0"),
    ("UUID", v"0.1.0"),
    ("UnalignedVectors", v"0.0.2"),
    ("UnicodeFun", v"0.1.0"),
    ("UnicodePlots", v"0.2.6"),
    ("UniqueVectors", v"0.4.0"),
    ("Unitful", v"0.2.6"),
    ("UnitfulIntegration", v"0.0.1"),
    ("UnitfulUS", v"0.0.1"),
    ("Units", v"0.2.6"),
    ("Unmarshal", v"0.0.4"),
    ("Unums", v"0.2.1"),
    ("VLFeat", v"0.0.6"),
    ("VML", v"0.1.0"),
    ("VSL", v"0.0.1"),
    ("VStatistic", v"1.0.0"),
    ("VT100", v"0.1.0"),
    ("ValidatedNumerics", v"0.9.0"),
    ("ValueDispatch", v"0.0.1"),
    ("ValueHistories", v"0.2.0"),
    ("ValueOrientedRiskManagementInsurance", v"0.0.4"),
    ("ValueSymbols", v"1.0.0"),
    ("VarianceComponentTest", v"0.1.3"),
    ("VariationalInequality", v"0.1.0"),
    ("Vectorize", v"0.0.1"),
    ("VectorizedRoutines", v"0.0.2"),
    ("Vega", v"0.7.0"),
    ("VegaLite", v"0.1.0"),
    ("VennEuler", v"0.0.1"),
    ("VideoIO", v"0.2.0"),
    ("Viridis", v"0.0.2"),
    ("VirtualArrays", v"0.1.1"),
    ("VisualRegressionTests", v"0.1.0"),
    ("Voronoi", v"0.1.0"),
    ("VoronoiCells", v"0.1.4"),
    ("VoronoiDelaunay", v"0.1.0"),
    ("Voting", v"0.0.1"),
    ("VulkanCore", v"1.0.0"),
    ("WAV", v"0.8.5"),
    ("WCS", v"0.2.0"),
    ("WCSLIB", v"0.2.1"),
    ("WORLD", v"0.4.1"),
    ("Wallace", v"0.0.1"),
    ("Watcher", v"0.1.1"),
    ("Watershed", v"0.5.0"),
    ("WaveletMatrices", v"0.1.0"),
    ("Wavelets", v"0.6.1"),
    ("WeakRefStrings", v"0.2.0"),
    ("Weave", v"0.4.4"),
    ("WebSockets", v"0.4.0"),
    ("Weber", v"0.5.4"),
    ("WeberCedrus", v"0.1.0"),
    ("WeberDAQmx", v"0.1.0"),
    ("Whittaker", v"1.0.2"),
    ("WiltonInts84", v"0.0.2"),
    ("WinRPM", v"0.2.5"),
    ("WinReg", v"0.2.0"),
    ("Winston", v"0.13.2"),
    ("WoodburyMatrices", v"0.2.2"),
    ("Word2Vec", v"0.0.3"),
    ("WordNet", v"0.1.0"),
    ("WorldBankData", v"0.0.5"),
    ("WriteVTK", v"0.6.1"),
    ("XClipboard", v"0.0.3"),
    ("XGBoost", v"0.2.0"),
    ("XKPasswd", v"0.0.2"),
    ("XMLDict", v"0.1.3"),
    ("XMLRPC", v"0.0.1"),
    ("XMLconvert", v"0.1.0"),
    ("XSV", v"0.0.2"),
    ("XSim", v"0.1.0"),
    ("Xpress", v"0.5.0"),
    ("Xtensor", v"0.0.11"),
    ("YAML", v"0.2.1"),
    ("YT", v"0.4.0"),
    ("Yelp", v"0.3.0"),
    ("Yeppp", v"0.2.0"),
    ("ZChop", v"0.1.2"),
    ("ZMQ", v"0.5.1"),
    ("ZVSimulator", v"0.0.0"),
    ("Zeros", v"0.0.1"),
    ("ZipCode", v"0.0.2"),
    ("ZipFile", v"0.5.0"),
    ("Zlib", v"0.1.12"),
    ("kNN", v"0.0.0"),
    ("mPulseAPI", v"1.0.7"),
    ])
