...
function gitmeta(pkgs::Dict{String,Package})
    ...
    for (pkg, p) in sort!(collect(pkgs), by=first)
        (pkg == "julia" || isempty(p.versions) || !(pkg in invenia_pkg_list)) && continue
        uuid = string(p.uuid)
        @info "Package [$uuid] $pkg"
        haskey(d, uuid) || (d[uuid] = Dict())
        haskey(s, uuid) || (s[uuid] = Dict())
        updated = false
        repo_path = joinpath(first(DEPOT_PATH), "clones", uuid)
        repo = nothing
        for (ver, v) in p.versions
            ...
            v"0.7" in v.requires["julia"].versions || v"1.0" in v.requires["julia"].versions || continue
            haskey(s[uuid], v.sha1) && continue
