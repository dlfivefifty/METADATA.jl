...
@info "Loading all package data from METADATA"
const pkgs = load_packages(METADATA_HOME)
...
# Read all Invenia private packages from the file names which differ between
# the `invenia` and `metadata-v2` Git branches on the remote that points to
# the Invenia private METADATA repository.
cd(METADATA_HOME) do
    remote = invenia_remote()
    metadata_v2 = remote * "/metadata-v2"
    invenia = remote * "/invenia"

    global invenia_pkg_list = String[]

    for line in eachline(`git diff --name-only $metadata_v2...$invenia -- "*/url"`)
        push!(invenia_pkg_list, line[1:prevind(line, findfirst(isequal('/'), line))])
    end
end

# For debugging
@info "Found the following Invenia packages:"
for pkg in invenia_pkg_list
    printstyled("  ", pkg, '\n'; color=Base.info_color())
end

# Note that this provides the ability to iterate over the dictionary of all
# uncurated packages and lazily filter to just Invenia packages without having
# to maintain a separate copy of the dictionary and update it alongside the
# main one.
invenia_pkgs = Iterators.filter(p->first(p) in invenia_pkg_list, pkgs)
